...
class monitoring_scenario extends scenario
{
    ...
    function get_s()
    {
        return (object)[
            ...
            'low' => [
                25 => 'тип документа',
                26 => 'Реквизиты закона, иного нормативного правового акта, устанавливающего требования к организации инфраструктуры либо предусматривающий право организации выполнять функции организаций инфраструктуры (Реквизиты документа (вид, наименование, дата, номер)',
                27 => 'Реквизиты закона, иного нормативного правового акта, устанавливающего требования к организации инфраструктуры либо предусматривающий право организации выполнять функции организаций инфраструктуры (Номер пункта / статьи / части документа)',
            ],
            'first' => [0, 9, 25],
            'data' => [],
            'rowcnt' => 0,
     ...
     * @return null|object
     */
    function get_ByCode($id = 0,$callback=null)
    {
        /** @var scaner $scaner */
        $_ = $this->get_s();
    ...
    function result2html($result)
    {
        ...
        // первая строка
        $r.= '<tr class=\'row_org row_reg_' . $regcode . '\'>
...
';
    ...
    function _nstring($s){
        return mb_substr(preg_replace('/\s+/su', '', $s),0,300,'utf-8');
    }
    ...
    function do_test_updates($updates = '', $date = '', $debug = true)
    {
        ...
        while ($row = $csv->nextRow()) {
            ...
            for ($i = 0; $i < count($res->values[$coord[0]]); $i++) {
                if (($coord[0] != 'sub'&&$coord[0] != 'low') || $this->_nstring(\UTILS::val($res, 'values|' . $coord[0] . '|' . $i . '|' . $coord[1])) == $this->_nstring( $oldval)) {
            ...
            if(!$found) {
                for ($i = 0; $i < count($res->values[$coord[0]]); $i++) {
                    if ($coord[0] != 'sub' || $this->_nstring( \UTILS::val($res, 'values|' . $coord[0] . '|' . $i . '|' . $coord[1])) == $this->_nstring( $newval)) {
                        $found = true;
                        break;
                    }
                }
                printf("new value already placed before %s-%s with  %s\n", $coord[0], $coord[1], $newval);
            }
            if (!$found) {
