...
class scaner
{
    ...
    function scan($reg)
    {
        ...
        do {
            ...
            if (!$this->found && !empty($this->handle) && !feof($this->handle)) { //3940043
                // $this->start=mb_strlen($this->buf,'8bit');
                $x = mb_strrpos($this->buf, self::NL, 0, '8bit');
                if ($this->till > 0 && $this->filestart + $x + 1 > $this->till) {
                    $this->position($this->till);
                    break;
                }
                if (false === $x) {
     ...
     * @param string $reg
     * @param bool $keeplasttillifmissed
     * @return $this
     */
    function until($reg = '', $keeplasttillifmissed = false)
    {
        if (empty($reg)) {
            $this->till = -1;
            return $this;
    ...
    function tillReg($tillreg)
    {
        $this->tillreg = $tillreg;
        return $this;
    }
    ...
    {
        // забираем себе финишную регулярку, чтобы ее можно было ставить во вложенных синтаксах
        $tillreg = $this->tillreg;
        $this->tillreg = false;
        $tf = -1;
        // so build a reg
        $idx = array('_skiped');
        ...
        $this->prepare(false);
        while (true) {
            // пытаемся применить regtill на имеющемся буфере.
            if (
                $tf < 0 && !empty($tillreg) // не был еще найден конец сканирования
                && $this->filestart != $prepare   // буфер сдвигался после последней проверки
            ) {
                if (preg_match($tillreg, $this->buf, $tm, PREG_OFFSET_CAPTURE, $this->start)) {
                    $tf = $tm[0][1];
                    if ($till < 0) {
                        $till = $tf + $this->filestart;
                    } else {
                        $till = min($till, $tf + $this->filestart);
                    }
                };
                $prepare = $this->filestart;
            }
            $skiped = '';
        ...
        if ($movepostotill && $till != $this->finish)
            $this->position($till);
        return $this;
    }
